...
@Component({
  selector: 'app-descargar-video',
  standalone: true,
  imports: [CommonModule, FormsModule, NgbToast],
  templateUrl: './descargar-video.html',
  styleUrls: ['./descargar-video.css']
})

export class DescargarVideoComponent {
  videoUrl: string = '';
  ...
  descargar() {
    ...
    this.videoService.descargarVideo(this.videoUrl).subscribe({
      next: (response: any) => {
