...
@Component({
  ...
  templateUrl: './descargar-audio.html',
  styleUrls: ['./descargar-audio.css']
})

export class DescargarAudioComponent {
  audioUrl: string = '';
  ...
  descargar() {
    ...
    this.audioService.descargarAudio(this.audioUrl).subscribe({
      next: (response: any) => {
