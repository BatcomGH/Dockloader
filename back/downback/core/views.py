...
@csrf_exempt 
def descargar_musica_api(request):
    if request.method == 'POST':
        try:
            print("¡Recibiendo petición POST para descargar!")
            ...
            ruta_archivo_descargado = video2audio.audDownMain(video_url)
...
def descargar_audio_api(request):
    if request.method == 'POST':
        try:
            ...
            dirDescargas = "/tmp"
            ruta_archivo_descargado = video2audio.audDownCore(video_url)

            if not ruta_archivo_descargado:
                response = JsonResponse({'error': 'Error al descargar el audio'}, status=500)
                response['Access-Control-Allow-Origin'] = '*'
                return response

            print(f"Audio listo en: {ruta_archivo_descargado}. Enviando a Angular...")
            ...
            if 'is not a valid URL' in str(e):
                ...
                error_response = JsonResponse({'error': f'Error al intentar descargar el audio: {str(e)}'}, status=500)
            error_response['Access-Control-Allow-Origin'] = '*'
            return error_response
